...
export const rentRequest = async (req, res) => {
  try {
    ...
    if (!offerID || !landOwner) {
      return res
        .status(400)
        .json({ error: "offerID and landOwner are required." });
    }

    await db.query(
      "INSERT INTO Farmers (ID) VALUES ($1) ON CONFLICT DO NOTHING",
      [farmerID]
    );
    ...
    await db.query(
      `
      INSERT INTO RentalDeals (offerID, landownerID, farmerID)
      VALUES ($1, $2, $3);
      `,
      values
    );
    ...
    if (err.code === "23505") {
      return res
        .status(409)
        .json({ error: "Rental deal already exists." });
    }
    console.error(err);
